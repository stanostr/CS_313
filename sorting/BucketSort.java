    ...
    public static void bucketSortEntries(Queue<Entry<Integer, ?>> queue, int N)
    {
        ...
        for(int j=0; j<N; j++)
        {
            if(B[j]!=null) {
                int seqSize = B[j].size();
                for (int k = 0; k < seqSize; k++)
                    queue.enqueue((Entry<Integer, ?>) B[j].dequeue());
            }
        }
    }

    /**
     * Bucket-sorts a DoublyLinkedList of Entries that have Integers as key
     * @param N range of possible Integer keys must between 0 and N-1
     */
    public static void bucketSortEntries(DoublyLinkedList<Entry<Integer, ?>> list, int N)
    {
        int n = list.size();
        if(n < 2)  return; //already sorted
        Queue[] B = new LinkedQueue[N];
        ...
        for(int j=0; j<N; j++)
        {
            if(B[j]!=null) {
                int seqSize = B[j].size();
                for (int k = 0; k < seqSize; k++)
                    list.addLast((Entry<Integer, ?>) B[j].dequeue());
            }
        }
    }
}
